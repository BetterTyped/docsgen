...
export const Particles = ({ className = "", quantity = 40, staticity = 50, ease = 50 }: ParticlesProps) => {
  ...
  const drawCircle = (circle: Circle, update = false) => {
    if (context.current) {
      ...
      context.current.fillStyle = document.body.classList.contains("dark")
        ? `rgba(80, 80, 80, ${alpha})`
        : `rgba(15, 23, 42, ${alpha})`;
  ...
  const animate = () => {
    ...
    if (!then.current || elapsed > fpsInterval) {
      clearContext();

...
export const Cards = () => {
  ...
  return (
    <div ref={containerRef} className="grid gap-6 sm:grid-cols-2 xl:grid-cols-2 mt-10">
      {sidebar.map((item, index) => {
        ...
        return (
          ...
          >
            <a href={item.link.path} style={{ textDecoration: "none" }}>
              <div className="flex flex-col p-6 h-full">
                <div className="flex items-center space-x-3 mb-3">
                  ...
                  >
                    {Img && <Img className="fill-white group-hover:fill-white/90 w-4 h-4" />}
                  </div>
                </div>
                <Title
                  size="none"
                  className="font-semibold !leading-[1.3]"
                  wrapperClass="flex flex-wrap !leading-3 mb-2"
                >
                  {item.name}
                </Title>

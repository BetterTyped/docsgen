...
export type DefaultPageOptions = {
...
export const DefaultPage: React.FC<PagePropsType> = (props) => {
  const { pluginOptions } = props;
  const { pages } = pluginOptions;
  const { mdx = true, intro = true, preview = true, returns = true, custom } = pages?.[PackagePages.DEFAULT] || {};

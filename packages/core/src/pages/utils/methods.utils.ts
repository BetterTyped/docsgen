...
const isFunctionReflection = (
  ...
  if (
    ...
    if (referenceType?.type?.type === "conditional") {
      ...
      if (referenceType.type?.falseType && isFunctionReflection(referenceType.type.falseType, reflectionsTree)) {
        return true;
      }
      if (referenceType.type?.checkType && isFunctionReflection(referenceType.type.trueType, reflectionsTree)) {
        return true;
      }
    }
    if (referenceType) {
